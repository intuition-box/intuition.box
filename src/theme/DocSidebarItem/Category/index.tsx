...
function CategoryLinkLabel({
  ...
  const iconNode = renderIcon(icon, styles.icon, styles.iconEmoji);
  return (
...
function DocSidebarItemCategoryCollapsible({
  ...
  const {items, label, collapsible, className, href, customProps} = item as any;
  const {
    docs: {
        ...
        <Link
          ...
          <CategoryLinkLabel
            ...
            icon={
              level <= 1
                ? undefined
                : customProps?.icon ?? (item as any).icon ?? 'folder'
            }
          />
        </Link>
        {href && collapsible && (
